...
from dataset import build_dataloaders, COMMON_WN, parse_spectrum_file, resample_spectrum
...
def parse_args():
    ...
    p.add_argument("--lab_data", type=str, default=None,
                   help="Path to flat lab data folder for optional post-training evaluation")
    p.add_argument("--plot_title", type=str, default="Model Evaluation",
...
def plot_metrics_table(y_true, y_pred, class_names, out_path, title="Performance Metrics"):
...
def plot_combined_confusion(online_true, online_pred, lab_true, lab_pred, class_names, out_path):
    img_online = _fig_to_image(_confusion_fig(online_true, online_pred, class_names, "Online Spectra Test Set"))
    img_lab    = _fig_to_image(_confusion_fig(lab_true,    lab_pred,    class_names, "Lab Spectra"))
...
def plot_combined_metrics(online_true, online_pred, lab_true, lab_pred, class_names, out_path):
    img_online = _fig_to_image(_metrics_fig(online_true, online_pred, class_names, "Online Spectra Test Set"))
    img_lab    = _fig_to_image(_metrics_fig(lab_true,    lab_pred,    class_names, "Lab Spectra"))
...
def _draw_metrics_in_ax(ax, y_true, y_pred, class_names):
    ...
    tbl = ax.table(cellText=rows, colLabels=col_labels,
                   bbox=[0.01, 0.06, 0.98, 0.88], cellLoc="center")
    tbl.auto_set_font_size(False)
    tbl.set_fontsize(8)
    for j in range(len(col_labels)):
        tbl[0, j].set_facecolor("#2166AC")
...
def plot_combined_2x2(online_true, online_pred, lab_true, lab_pred, class_names, out_path, title):
    ...
    online_report = classification_report(online_true, online_pred, target_names=class_names, output_dict=True)
    lab_report    = classification_report(lab_true,    lab_pred,    target_names=class_names, output_dict=True)
...
def main():
    ...
    plot_confusion(y_true, y_pred, class_names, out / "confusion_matrix_online.png",
                   title="Online Spectra Test Set Confusion Matrix")
    plot_metrics_table(y_true, y_pred, class_names, out / "metrics_table_online.png",
                       title="Online Spectra Test Set Performance Metrics")

    if args.lab_data:
        lab_true, lab_pred = evaluate_lab_data(
            args.lab_data, model, scaler, class_names, device,
        )
        ...
        plot_confusion(lab_true, lab_pred, class_names, out / "confusion_matrix_lab.png",
                       title="Lab Spectra Confusion Matrix")
        plot_metrics_table(lab_true, lab_pred, class_names, out / "metrics_table_lab.png",
                           title="Lab Spectra Performance Metrics")
        plot_combined_confusion(y_true, y_pred, lab_true, lab_pred, class_names,
                                out / "combined_confusion.png")
        plot_combined_metrics(y_true, y_pred, lab_true, lab_pred, class_names,
                              out / "combined_metrics.png")
        plot_combined_2x2(y_true, y_pred, lab_true, lab_pred, class_names,
                          out / "combined_2x2.png", title=args.plot_title)
