...
function train_model!(model, chunk_paths::Vector{String},
    ...
    for e in 1:epochs
        ...
        for path in shuffle(chunk_paths)
            ...
            train_loader = DataLoader((Xtr_gpu, Ytr_gpu), batchsize=256, shuffle=true)
            println("train data loader done")
            for (Xb, Yb) in train_loader
                println("loop init")
                # Xb, Yb = dev(Xb), dev(Yb)
                # println("moved to gpu")
                # waits quite long here for some reason before it says gradient done. maybe some sort of cold start issue
                # gradient calculation still takes the longest by far in this loop.
